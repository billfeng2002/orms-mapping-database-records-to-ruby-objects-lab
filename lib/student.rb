...
class Student
  ...
  def self.first_X_students_in_grade_10(num)
    ...
    students=DB[:conn].execute(sql, num).map{|student| self.new_from_db(student)}
  end
  ...
  def self.all_students_in_grade_X(grade)
    ...
    students=DB[:conn].execute(sql, grade).map{|student| self.new_from_db(student)}
  end
  def save
    sql = <<-SQL
